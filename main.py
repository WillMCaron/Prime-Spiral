...
def prime(num):
  ...
  for i in range(2, int(num/2)+1):
    # if it is divisible by the number
    if (num%i)==0:
      return False
  return True
...
for i in range(numSteps):
  ...
  if prime(count):
    ...
    pygame.draw.circle(windowSurface, WHITE, (x, y), STEPS/3, 0)
  # delay
  pygame.display.update()
  numDisplay = font.render(str(count),1,GREEN)
  sleep(DELAY)
...
while True:
  ...
  for i in range(numSteps):
    ...
    if x > (SCR_WIDTH - STEPS*2) and y > (SCR_LENGTH-STEPS*2):
      #pygame.quit()
      #sys.exit()
      a = input()
      pygame.quit()
